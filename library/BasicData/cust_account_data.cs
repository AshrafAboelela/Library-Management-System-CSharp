...
{
    public partial class cust_account_data : Form
    {
        public cust_account_data()
        {
            InitializeComponent();
        }
        bool _addnew = false;
        ...
        void bindcombo()
        {
            Retrivedata.fillcust(cmb_search);
        }
        private void btn_new_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("هل تريد سداد مبلغ عميل", "جديد", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                _addnew = true;
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            if (_addnew == true)
            {
                if (MessageBox.Show("هل انت متاكد من الحفظ", "حفظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show("لا يوجد مبلغ متبقى على العميل");
                        return;
                    }
                    ...
                    Retrivedata.executenonquery("sp_cust_accountsinsert",
                       new Retrivedata.myParamter("@cust_id", SqlDbType.Int, int.Parse(cmb_search.SelectedValue.ToString())),
                       new Retrivedata.myParamter("@payed_value", SqlDbType.Decimal, txt_payed.Text),
                       new Retrivedata.myParamter("@date_payed", SqlDbType.Date, date_convert.DateTime.Date),
        ...
        private void cust_account_data_Load(object sender, EventArgs e)
        {
            bindcombo();
        }
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            if (cmb_search.Text != "")
            {
                ...
                dt = Retrivedata.executequery("sp_customersselectbyid", CommandType.StoredProcedure,
                    new Retrivedata.myParamter("@cust_id", SqlDbType.Int, int.Parse(cmb_search.SelectedValue.ToString())));
                txt_remain.Text = dt.Rows[0]["cust_debit"].ToString();
            }
        }
